 ...
 * por lo que al interceptarlos se utiliza esta clase para relanzarlos y
...
class MoodleException : ResponseStatusException {
   ...
   * poder generar un token. El endpoint para obtener el token siempre responde
   * en formato JSON, por ello no hace falta mapear propiedades para
   * `XmlMapper`.
   *
   * Referencia:
  ...
  /**
   * Estructura JSON que devuelven los servicios REST de Moodle ante errores. El
   ...
   * que las propiedades del `data class` se mapean a los campos del XML
   * mediante anotaciones; no hace falta mapearlas al JSON porque se utilizan
