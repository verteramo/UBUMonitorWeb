...
import { SettingsStore, ThemeMode } from '@core/store/settings.store';
...
export class ThemeToggleComponent {
  ...
  currentIcon = computed(() => this.icons[this.settingsStore.$value().theme]);

  nextThemeTooltip = computed(() => {
    const current = this.settingsStore.$value().theme;
